...
async function main() {
  try {
    ...
    var us10y = await fetchFRED('DGS10');
    ...
    // 일본 10년물 (월별 최신)
    var jp10y = await fetchFRED('IRLTLT01JPM156N');
    ...
    var wtregen  = await fetchFRED('WTREGEN');      // 은행 긴급차입 (백만달러)
    var m2       = await fetchFRED('M2SL');         // M2 통화량 (십억달러)

    var us10yVal  = parseFloat(us10y  ? us10y.value  : 0);
    ...
    var jp10yVal  = parseFloat(jp10y  ? jp10y.value  : 0);
    ...
    var resVal     = parseFloat(reserves ? reserves.value : 0);   // 십억달러
    ...
    var date = us10y ? us10y.date : new Date().toISOString().slice(0, 10);
