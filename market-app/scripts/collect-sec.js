...
async function parseFilingXml(accNum, filed) {
  // 인덱스에서 infotable 파일 찾기
  var idxUrl = 'https://www.sec.gov/Archives/edgar/data/' + parseInt(CIK) + '/' + accNum + '/' + accNum + '-index.json';
  var idxRes = await fetch(idxUrl, { headers: { 'User-Agent': 'market-app contact@example.com' } });

  var xmlUrl = null;
  if (idxRes.ok) {
    var idxData = await idxRes.json();
    var files = (idxData.directory && idxData.directory.item) || [];
    for (var f of files) {
      if (f.name && f.name.toLowerCase().includes('infotable')) {
        xmlUrl = 'https://www.sec.gov/Archives/edgar/data/' + parseInt(CIK) + '/' + accNum + '/' + f.name;
        break;
      }
    }
  }
  ...
  if (!xmlUrl) {
    // 폴백: 일반적인 파일명 시도
    xmlUrl = 'https://www.sec.gov/Archives/edgar/data/' + parseInt(CIK) + '/' + accNum + '/infotable.xml';
  }
  ...
  // infoTable 블록 파싱
  var holdings = {};
  var regex = /<infoTable>([\s\S]*?)<\/infoTable>/gi;
  var match;
  while ((match = regex.exec(xml)) !== null) {
    var block = match[1];
    var nameMatch = block.match(/<nameOfIssuer>(.*?)<\/nameOfIssuer>/i);
    var valueMatch = block.match(/<value>(.*?)<\/value>/i);
    var sharesMatch = block.match(/<sshPrnamt>(.*?)<\/sshPrnamt>/i);
...
async function main() {
  try {
    ...
    // ── nps-sec13f.json 저장 (TOP 10 보유) ──
    var top10 = currentItems.slice(0, 10).map(function(item, idx) {
      return {
        rank: idx + 1,
