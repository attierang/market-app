...
async function main() {
  try {
    ...
    while (page <= totalPage && page <= 200) {
      ...
      const nps = (data.list || []).filter(i =>
        i.flr_nm && i.flr_nm.includes('국민연금') &&
        i.report_nm && (i.report_nm.includes('대량보유') || i.report_nm.includes('주요주주') || i.report_nm.includes('임원'))
      );

      for (const item of nps) {
        if (item.report_nm.includes('대량보유')) majorList.push(item);
        else eleList.push(item);
      }
    ...
    for (const item of majorList.slice(0, 50)) {
      ...
      majorDetailed.push({
        corp_name: item.corp_name, stock_code: item.stock_code,
        report_nm: item.report_nm, rcept_dt: item.rcept_dt, flr_nm: item.flr_nm,
        ratio: ratio, change: change, stock_count: Math.abs(cnt),
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ updated_at: new Date().toISOString(), count: majorDetailed.length, data: majorDetailed }, null, 2), 'utf8'
    );
    fs.writeFileSync(
      path.join(dataDir, 'nps-ele.json'),
      JSON.stringify({ updated_at: new Date().toISOString(), count: eleDetailed.length, data: eleDetailed }, null, 2), 'utf8'
    );
