...
// quoteSummary로 PER 단건 조회 (디버그 모드)
async function fetchPE(symbol, cookie, crumb) {
  try {
    ...
    if (!res.ok) return { pe: null, _dbg: 'status=' + res.status + ' body=' + raw.slice(0, 200) };
    const data = JSON.parse(raw);
    const result = data?.quoteSummary?.result?.[0];
    const pe = result?.summaryDetail?.trailingPE?.raw ?? null;
    return { pe, _dbg: 'ok keys=' + Object.keys(result?.summaryDetail || {}).join(',').slice(0, 100) };
  } catch(e) { return { pe: null, _dbg: 'err=' + e.message }; }
}
...
export default async function handler(req) {
  ...
  try {
    ...
    const [quoteRes, ...peResults] = await Promise.all([
      fetch(quoteUrl, {
        headers: { 'User-Agent': UA, 'Cookie': cookie, 'Accept': 'application/json', 'Referer': 'https://finance.yahoo.com' }
      }),
      ...symbolList.map(s => fetchPE(s, cookie, crumb))
    ]);
    ...
    return new Response(JSON.stringify({ ok: true, data: result, _peDbg: peDbg }), { headers: CORS });
